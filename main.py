...
def config_delete(config_type: str, config_id: str, db: Session = Depends(get_db)):
    cs = ConfigStore(db)
    if config_type in ("llm", "drawthings"):
        if cs.is_referenced(config_id):
            raise HTTPException(status_code=400, detail="该配置仍被项目使用，无法删除")
...
def micro_work_page(work_id: str, db: Session = Depends(get_db)):
...
def micro_session_page(work_id: str, session_id: str, db: Session = Depends(get_db)):
    """作品 + 选中会话的消息历史。"""
    work = db.get(MicroWork, work_id)
...
def micro_work_delete(work_id: str, db: Session = Depends(get_db)):
...
def micro_session_delete(work_id: str, session_id: str, db: Session = Depends(get_db)):
...
async def micro_chat(request: Request, work_id: str, session_id: str, db: Session = Depends(get_db)):
    ...
    if raw_images:
        if not (llm_cfg and llm_cfg.supports_vision == "yes"):
            raise HTTPException(status_code=400, detail="当前 LLM 配置不支持图片")
...
async def _micro_stream(db: Session, session: MicroSession, llm_cfg, dt_cfg,
    ...
    async def _run(out: asyncio.Queue):
        try:
            ...
            await out.put(("error", {"message": f"对话失败：{e}"}))
            await out.put(("done", {}))
        finally:
...
def project_delete(project_id: str, db: Session = Depends(get_db)):
    """删除创作（含全部章节与媒体文件）。"""
    project = pipeline.get(db, project_id)
    if not project:
        raise HTTPException(status_code=404, detail="项目不存在")
...
def project_view(project_id: str, db: Session = Depends(get_db)):
    ...
    data = {
        "project": {
            ...
            "llm_name": llm_cfg.name if llm_cfg else "未知配置",
            "dt_name": dt_cfg.name if dt_cfg else "未知配置",
        },
...
async def project_action(request: Request, project_id: str, db: Session = Depends(get_db)):
    ...
    try:
        if step == "scope":
            project = await pipeline.step_scope(db, project)
        elif step == "arc":
            project = await pipeline.step_arc(db, project)
        elif step == "chapters":
            project = await pipeline.step_chapters(db, project)
        elif step == "script":
            project = await pipeline.step_script(db, project)
        elif step == "generate":
            project = pipeline.step_generate(db, project)
        else:
            raise HTTPException(status_code=400, detail=f"未知步骤: {step}")
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"【{step}】失败：{e}")
    return {"ok": True}


@app.post("/api/projects/{project_id}/gen/{index}")
def project_gen_single(project_id: str, index: int, db: Session = Depends(get_db)):
    """生成指定章节（图/视频）。"""
    project = pipeline.get(db, project_id)
    if project is None:
        raise HTTPException(status_code=404, detail="项目不存在")
    ...
    try:
        project = pipeline.step_generate(db, project, index=index)
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"【生成第{index+1}章】失败：{e}")
    return {"ok": True}
...
def project_regenerate(project_id: str, index: int, db: Session = Depends(get_db)):
    """重新生成指定章节。"""
    project = pipeline.get(db, project_id)
    if project is None:
        raise HTTPException(status_code=404, detail="项目不存在")
    ...
    if not (0 <= index < len(chapters)):
        raise HTTPException(status_code=404, detail="章节不存在")
    try:
        project = pipeline.regenerate(db, project, index)
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"【重生成第{index+1}章】失败：{e}")
    return {"ok": True}


# ---------------- 首图 / 总纲 ----------------
@app.post("/api/projects/{project_id}/first-image")
def project_first_image_upload(project_id: str, file: UploadFile = File(...),
                               db: Session = Depends(get_db)):
    """上传首图（作为第 1 章参考图）。"""
    project = pipeline.get(db, project_id)
    if project is None:
    ...
    if ext not in (".png", ".jpg", ".jpeg", ".webp", ".gif"):
        raise HTTPException(status_code=400, detail="请上传图片文件（png/jpg/webp/gif）")
...
async def project_first_image_generate(request: Request, project_id: str,
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"【生成首图】失败：{e}")
    return {"ok": True}

...
- **新建表单 AI 生成标题/主题**：新建页有 **「✨ AI 生成标题/主题」** 按钮，打开**多轮、流式**对话；
  AI 会先问几个问题帮你把想法定清楚，随后通过 function call（`set_fields`）**把标题与主题直接写进表单**
  （可继续对话 / 让它调整）。
...
│   ├── api_idea.py      # 新建创作「AI 生成标题/主题」路由（/api/idea/*）+ SSE 传输层
│   ├── idea_agent.py    # 新建创作创意引擎（多轮 + 流式 + set_fields function call）

...
（需在对应 DrawThings 配置里勾选「支持参考图片」；未勾选按文生图 / 文生视频生成）。
...
连续性参考需在该作品的 DrawThings 配置里**勾选「支持参考图片」**（图生图 / 图生视频）；未勾选按文生图 / 文生视频生成。
...
- **DrawThings 配置**（仅 gRPC，app 里 API server 设为 gRPC）：
  ...
  **支持参考图片**（`ref_image` / `ref_video`，默认**不勾选**）：图像 / 视频各自声明能否图生图 / 图生视频；
  勾选后漫画按上一张图、短剧按上一视频末帧作 `init_image` 传入，未勾选一律文生图 / 文生视频（未填对应模型时置灰并归零）。
...
- 连续性参考：漫画沿用上一张图、短剧沿用上一段视频末帧（客户端自动抽取）；
  需在配置里勾选「支持参考图片」（`ref_image` / `ref_video`），未勾选按文生图 / 文生视频生成。
